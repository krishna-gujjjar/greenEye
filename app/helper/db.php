...
abstract class DB
{
    ...
    {
        $this->connect = mysqli_connect(DB_HOST, DB_USER, DB_PASS) or die('Database not Connected');
        if ($this->connect) {
            $createDB = mysqli_query($this->connect, 'CREATE DATABASE IF NOT EXISTS ' . DB_NAME);
            if ($createDB) {
                $selectDB = mysqli_select_db($this->connect, DB_NAME);
                if ($selectDB) {
                    ...
                    $createUser = mysqli_query($this->connect, $createUser) or exit('Table Not Create');
                    if ($createUser) {
                        $checkIndex = "SHOW INDEX FROM `gReeneye_uSEr`";
                        $checkIndex = mysqli_query($this->connect, $checkIndex) or exit('Cant Check Index');
                        $rowss = mysqli_num_rows($checkIndex);
                        if ($rowss < 2) {
                            $indexDB = "ALTER TABLE `gReeneye_uSer`
                                ADD KEY `rID` (`gReeneye_uriD`)";
                            $indexDB = mysqli_query($this->connect, $indexDB) or exit('Cant Set Index');
                        }
                        $checkData = "SELECT * FROM `gReeneye_uSer` WHERE `gReeneye_uiD` = 1";
                        $checkData = mysqli_query($this->connect, $checkData) or exit('Cant Show Data');
                        if ($checkData) {
                            ...
                            if ($row > 0) {
                                ...
                                $insertUser = mysqli_query($this->connect, $insertUser) or exit('Data not Insert in Table');
                                if ($insertUser) {
                                    return true;
                                }
                            }
